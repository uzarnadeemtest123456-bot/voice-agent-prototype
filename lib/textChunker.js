...
export class TextChunker {
  ...
  tryExtractChunks() {
    while (this.buffer.length > 0) {
      ...
      // Fallback: If buffer exceeds 80 chars, cut at whitespace (more aggressive)
      if (this.buffer.length > 80) {
        // Find last whitespace before 80 chars
        let cutPoint = 80;
        for (let i = 80; i >= 0; i--) {
  ...
  resetTimeout() {
    ...
    // Dispatch after 400ms if nothing else triggers (reduced for faster response)
    this.timeoutId = setTimeout(() => {
      if (this.buffer.trim().length >= 30 && !this.streamEnded) {
  ...
  dispatchChunk(chunk) {
    ...
    console.log(`📝 Chunk extracted (${chunk.length} chars): "${chunk.substring(0, 50)}..."`);
    this.onChunk(chunk);
    this.lastDispatchTime = Date.now();
  }
