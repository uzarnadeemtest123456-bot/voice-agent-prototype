...
export class TextChunker {
  ...
  tryExtractChunks() {
    while (this.buffer.length > 0) {
      ...
      if (newlineIndex !== -1) {
        ...
        if (chunk.length >= 20 || this.streamEnded) {
          this.dispatchChunk(chunk);
      ...
      // Fallback: If buffer exceeds 160 chars, cut at whitespace
  ...
  resetTimeout() {
    ...
    // Dispatch after 800ms if nothing else triggers
    this.timeoutId = setTimeout(() => {
      if (this.buffer.trim().length >= 20 && !this.streamEnded) {
