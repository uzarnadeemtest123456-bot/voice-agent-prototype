...
/**
 * Parse plaintext streaming (fallback when not using SSE)
 * Just returns chunks as-is for progressive text
 * @param {string} chunk
 * @returns {Array} Array with single delta event
 */
export function parsePlaintextStream(chunk) {
  if (!chunk) return [];
  return [{
    event: 'delta',
    data: { text: chunk }
